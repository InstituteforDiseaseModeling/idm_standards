> **Historical note**: this is the prompt that originally created the engineering-quality skills (then named `eng-quality-checker`/`eng-quality-fixer`, now `audit-code`/`fix-code`). File names and paths below reflect that era; see `update_prompt.md` for the current update process.
