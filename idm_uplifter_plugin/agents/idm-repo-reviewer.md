...
description: >
  ...
  markdown findings block. Dispatched in parallel by the idm-code-uplifter skill,
  once per run, alongside the per-file reviewers. Read-only — never modifies code.

  Examples:
  <example>
  Context: idm-code-uplifter skill is running and dispatching the repo-level review in parallel with file waves
...
safety:
  ...
  reproducible:
    dependencies: "Dependencies are specified; an environment/lock file is also included for non-library code."
