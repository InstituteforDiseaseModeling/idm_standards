...
# Tier 3 one-off analysis script: simulate an SIR outbreak, fetch a parameter, and plot.

# Deliberate flaws for the config eval:
...
def fetch_beta(region):
    r = requests.get("https://example.com/params", headers={"Authorization": "Bearer " + API_KEY})
    return r.json()["beta"]
